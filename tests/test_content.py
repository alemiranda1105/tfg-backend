...
from .mocked_test_data import content_data_test, mocked_jwt, mocked_jwt_admin
...
def test_get_all():
    response = client.get("idsemapi/content/")
    assert response.status_code == 200
    data = response.json()
...
def test_get_by_title():
    response = client.get("idsemapi/content/{}".format(content_data_test[0]['title']))
    assert response.status_code == 200
    data = response.json()
...
def test_get_by_title_error():
    response = client.get("idsemapi/content/error")
    assert response.status_code == 404
...
def test_update_content():
    ...
    response = client.put(
        "idsemapi/content/{}".format(inserted_content[0]),
        headers={
            'Authorization': 'Bearer {}'.format(mocked_jwt_admin)
        },
        json=updated
